...
async def handle(payload: Payload, correlation_id: str, session, actor: ToolActor | None = None) -> ToolResponse:
    ...
    settings = await NotificationSettingsService.set_weekly(session, actor.owner_user_id, enabled=False)
    return ToolResponse.ok(
        correlation_id=correlation_id,
        data={"owner_id": actor.owner_user_id, "weekly_enabled": settings.weekly_enabled, "message": "Еженедельный отчёт выключен."},
