...
async def handle(payload: Payload, correlation_id: str, session) -> ToolResponse:
    ...
    data = {
        "counts": {
            ...
            "missing_photo": len(missing_photo),
            "missing_price": len(missing_price),
            "unpublished": len(unpublished),
        },
        "out_of_stock": serialize(out_of_stock),
        "low_stock": serialize(low_stock),
        "missing_photo": serialize(missing_photo),
        "missing_price": serialize(missing_price),
        "unpublished": serialize(unpublished),
    }
    provenance = ToolProvenance(
        sources=["ownerbot_demo_products", "local_demo"],
        window=None,
        filters_hash=f"low_stock_lte:{payload.low_stock_lte};limit:{payload.limit}",
    )
