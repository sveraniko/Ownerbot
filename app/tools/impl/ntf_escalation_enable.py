...
async def handle(payload: Payload, correlation_id: str, session, actor: ToolActor | None = None) -> ToolResponse:
    ...
    settings = await NotificationSettingsService.set_escalation_enabled(session, actor.owner_user_id, enabled=True)
    return ToolResponse.ok(
        correlation_id=correlation_id,
        data={"owner_id": actor.owner_user_id, "escalation_enabled": bool(settings.escalation_enabled), "message": "Escalation включена."},
